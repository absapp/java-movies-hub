...
public class MoviesApiTest {
    ...
    void getMovies_whenNotEmpty_returnsMoviesArray() throws Exception {
        ...
        List<Movie> moviesFromResponse = gson.fromJson(resp.body(),
                new ListOfMoviesTypeToken().getType());

        assertNotNull(moviesFromResponse);
        assertFalse(moviesFromResponse.isEmpty());
        assertTrue(moviesFromResponse.stream()
                .anyMatch(m -> "Test Movie".equals(m.getTitle())));
    }

    @Test
    void postMovies_whenValidData_returnsCreatedMovie() throws Exception {
        String newMovieJson = "{\"title\": \"Inception\", \"year\": 2010}";
        ...
        List<Movie> allMovies = gson.fromJson(getResp.body(),
                new ListOfMoviesTypeToken().getType());
    ...
    void postMovies_whenWrongContentType_returns415() throws Exception {
        ...
        assertEquals(415, resp.statusCode(),
                "Должен вернуться 415 Unsupported Media Type");

        String body = resp.body();
        assertTrue(body.contains("error"), "Ответ должен содержать поле 'error'");
        assertTrue(body.contains("Неподдерживаемый тип содержимого")
    ...
    void postMovies_whenEmptyTitle_returns422WithDetails() throws Exception {
        String newMovieJson = "{\"title\": \"\", \"year\": 2010}";
        ...
        assertTrue(allMovies.isEmpty(),
                "Фильм с пустым названием не должен быть добавлен");
    }

    @Test
    void postMovies_whenTitleTooLong_returns422WithDetails() throws Exception {
        String longTitle = "A".repeat(101);
        String newMovieJson = "{\"title\": \"" + longTitle + "\", \"year\": 2010}";
    ...
    void postMovies_whenYearTooOld_returns422WithDetails() throws Exception {
        String newMovieJson = "{\"title\": \"Old Movie\", \"year\": 1887}";
    ...
    void postMovies_whenYearTooFuture_returns422WithDetails() throws Exception {
        ...
        String newMovieJson = "{\"title\": \"Future Movie\", \"year\": "
                + tooFutureYear + "}";
        ...
        String body = resp.body();
        assertTrue(body.contains(String.valueOf(currentYear + 1))
                        || body.contains("год"),
    ...
    void postMovies_whenInvalidJson_returns422() throws Exception {
        String invalidJson = "{\"title\": \"Incomplete\", \"year\": 2010";
    ...
    void getMovieById_whenExists_returnsMovie() throws Exception {
        String newMovieJson = "{\"title\": \"The Matrix\", \"year\": 1999}";
        ...
        assertEquals("The Matrix", retrievedMovie.getTitle(),
                "Название должно совпадать");
    ...
    void deleteMovieById_whenExists_returns204AndRemovesMovie() throws Exception {
        String newMovieJson = "{\"title\": \"Movie to Delete\", \"year\": 2020}";
        ...
        HttpResponse<String> getBeforeResp = client.send(getBeforeReq,
                HttpResponse.BodyHandlers.ofString());
        assertEquals(200, getBeforeResp.statusCode(),
                "Фильм должен существовать до удаления");
        ...
        assertEquals(204, deleteResp.statusCode(),
                "DELETE /movies/{id} должен вернуть 204 No Content");
        ...
        HttpResponse<String> getAfterResp = client.send(getAfterReq,
                HttpResponse.BodyHandlers.ofString());
        assertEquals(404, getAfterResp.statusCode(),
                "После удаления фильм не должен существовать");
        ...
        HttpResponse<String> getAllResp = client.send(getAllReq,
                HttpResponse.BodyHandlers.ofString());
        List<Movie> allMovies = gson.fromJson(getAllResp.body(),
                new ListOfMoviesTypeToken().getType());
    ...
    void deleteMovieById_whenAlreadyDeleted_returns404() throws Exception {
        String newMovieJson = "{\"title\": \"Double Delete Movie\", \"year\": 2021}";
        ...
        HttpResponse<String> firstDeleteResp = client.send(firstDeleteReq,
                HttpResponse.BodyHandlers.ofString());
        assertEquals(204, firstDeleteResp.statusCode(),
        ...
        HttpResponse<String> secondDeleteResp = client.send(secondDeleteReq,
                HttpResponse.BodyHandlers.ofString());
        assertEquals(404, secondDeleteResp.statusCode(),
    ...
    void getMoviesByYear_whenMoviesExist_returnsFilteredList() throws Exception {
        ...
        assertEquals(200, resp.statusCode(),
                "GET /movies?year=2020 должен вернуть 200");

        String contentType = resp.headers().firstValue("Content-Type").orElse("");
        assertEquals("application/json; charset=UTF-8", contentType);

        List<Movie> filteredMovies = gson.fromJson(resp.body(),
                new ListOfMoviesTypeToken().getType());
    ...
    void getMoviesByYear_whenNoMoviesForYear_returnsEmptyArray() throws Exception {
        ...
        assertEquals(200, resp.statusCode(),
                "Должен вернуться 200 даже если фильмов нет");
    ...
    void getMoviesByYear_whenYearParameterMissing_returnsAllMovies() throws Exception {
        ...
        List<Movie> allMovies = gson.fromJson(resp.body(),
                new ListOfMoviesTypeToken().getType());
